"""
Endpoint logs enrichi — Module 3.
Ajoute le pipeline de correlation apres chaque ingestion.
Remplace ou complete app/api/v1/endpoints/logs.py.
"""
import uuid
...
async def _run_correlation(
    ...
    """
    Pipeline de correlation post-ingestion.
    Execute en arriere-plan pour ne pas bloquer la reponse HTTP.
    """
    try:
        ...
        # Ne jamais bloquer l'ingestion si la correlation echoue
...
async def ingest_log(
    ...
    """
    Ingestion d'un log unique.
    Pipeline : normalisation -> PostgreSQL -> Elasticsearch -> correlation (background).
    """
    entry = await log_service.ingest_log(payload, db, es)

    # Correlation en arriere-plan (ne bloque pas la reponse)
    background_tasks.add_task(_run_correlation, entry, db)

    await log_audit("log_ingested", db, user_id=current_user.id,
                    target=f"log#{entry.id}", ip=request.client.host if request.client else None)
    await db.commit()
...
async def ingest_batch(
    ...
    """
    Ingestion par lot (max 500).
    Correlation lancee en arriere-plan apres le commit.
    """
    result = await log_service.ingest_batch(payload, db, es)

    # Correlate chaque log du batch (background)
    # On recharge les IDs depuis PG pour avoir les objets complets
...
async def _correlate_batch(batch_id: str, db: AsyncSession) -> None:
    """Correlation de tous les logs d'un batch (arriere-plan)."""
    try:
        result = await db.execute(
            select(LogEntry).where(LogEntry.batch_id == batch_id)
        )
...
async def flag_log(
    ...
    await log_audit("log_flagged", db, user_id=current_user.id,
                    target=str(log_id), ip=request.client.host if request.client else None)
    await db.commit()
    return LogDetail.model_validate(entry)
