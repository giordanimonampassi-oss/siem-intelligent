...
from core.constants import PlaybookMode, PlaybookStatus, PlaybookType, LogSeverity
...
async def execute_playbook(
    ...
    # Mode SEMI_AUTO / CONFIRM : verifier le delai
    if execution.mode in (PlaybookMode.SEMI_AUTO.value, "confirm"):
    ...
    try:
        ...
        if execution.playbook == PlaybookType.BLOCK_IP.value:
            ...
            # Webhook + email en parallele
            webhook_r, email_r = await asyncio.gather(
                _send_webhook(alert),
                _send_email(alert),
                return_exceptions=True,
            )
            result = {
                "webhook": webhook_r if not isinstance(webhook_r, Exception) else str(webhook_r),
                "email":   email_r   if not isinstance(email_r,   Exception) else str(email_r),
            }
...
async def trigger_auto_playbooks(
    ...
    if alert.severity == LogSeverity.CRITICAL.value:
        ...
        if alert.username:
            pb = PlaybookExecution(
                ...
                mode=PlaybookMode.SEMI_AUTO.value,
